...
def get_and_mark_frames(disagreements, traces, agent_position=(0, 0), actions=[None, None]):
...
def mark_trajectory_step(img, rel_pos, ref_pos, temp_img, color=255, thickness=2):
    img2 = img.copy()
...
def get_relative_position(trace, trajectory, da_index, relative_idx):
    a1_obs = np.array([trace.positions[x].destination for x in
                       trajectory.a1_states[da_index + 1:]])
    a2_obs = np.array([x.position.destination for x in
